...
export const architectureAssessment = {
  ...
  steps: [
    ...
    },
    {
      ...
      options: ["The kernel map $\\phi$ destroys positional information, so linear attention cannot represent token ordering and fails on tasks requiring position-aware retrieval from context", "Linear attention cannot be parallelized during training due to the sequential recurrence structure, making it slower than standard attention despite lower asymptotic complexity", "Linear attention uses fewer learnable parameters than standard attention, reducing the model's representational capacity below the threshold needed for retrieval-heavy tasks", "The kernel map $\\phi$ compresses key-query interactions into a fixed-size $d \\times d$ matrix, which cannot store and retrieve arbitrary token-level associations from long contexts"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["They use convolutions instead of any recurrence, making them purely feedforward with fixed-length receptive fields determined by the kernel size at each layer", "They replace attention with graph neural networks over token dependency trees, leveraging syntactic structure for efficient long-range context modeling", "They model sequences via a continuous-time dynamical system $\\dot{h}(t) = Ah(t) + Bx(t)$, discretized for efficiency, enabling linear-time processing with a fixed-size state", "They use external differentiable memory modules to store long-range dependencies, reading and writing to a memory bank at each processing step in the sequence"],
      correct: 2,
    ...
    },
    {
      ...
      options: [
        "Different layers use different hidden dimensions to save compute, with narrower layers handling simpler transformations and wider layers dedicated to complex reasoning steps",
        "A learned router decides per token whether to apply the full Transformer block or skip it via residual bypass, so easy tokens use less compute than hard tokens",
        "The model uses a variable number of layers that changes dynamically during training, adapting depth based on the current loss landscape and gradient signal statistics",
        "Attention heads at deeper layers are progressively pruned to reduce compute, since empirically the deeper layers contribute less to the final token prediction accuracy"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: ["Variance penalization would be too expensive to compute at scale, requiring tracking of running statistics across all experts per batch and adding prohibitive overhead", "The $f_i \\cdot p_i$ formulation also regularizes expert weights directly, preventing any single expert from developing disproportionately large parameters relative to others", "Variance penalization would force all experts toward identical representations, eliminating the specialization that gives MoE models their capacity advantage over dense models", "The $f_i \\cdot p_i$ product is differentiable w.r.t. the router logits (through $p_i$), whereas $f_i$ alone involves a non-differentiable argmax/top-k selection step"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Attention layers are placed only in the first few layers to establish position encoding and token interactions, while recurrent layers handle all remaining computation efficiently", "Recurrent layers are only needed during inference for efficient KV-cache management and fast autoregressive decoding, while training uses pure attention throughout the network", "Attention provides precise in-context retrieval while recurrent layers provide efficient long-range context compression — the hybrid achieves both at lower total cost than pure attention", "The combination enables the model to process images and text simultaneously, routing visual tokens through attention layers and language tokens through the recurrent layers"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Early-layer representations live in a different space than what the final output head expects, requiring separate per-layer output heads or alignment techniques that add training complexity", "Early layers have too few parameters to make accurate predictions, because the parameter count scales linearly with layer index in standard Transformer architecture designs", "Early exit requires a different tokenizer at each exit point, because intermediate representations encode tokens at different granularity levels than the final layer representations", "Gradient computation is impossible with early exit during training, because truncating the computational graph at the exit point blocks backpropagation to all earlier layers"],
      correct: 0,
