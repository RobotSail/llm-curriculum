  ...
  steps: [
    {
      ...
      options: [
        "Soft targets are easier to compute because the temperature-scaled softmax requires fewer floating-point operations than the standard argmax function, reducing overall training cost per batch significantly",
        "The teacher's distribution over **incorrect classes** encodes similarity structure (\"dark knowledge\") — e.g., assigning 0.05 to 'cat' and 0.001 to 'car' reveals inter-class relationships",
        "Soft targets prevent the student from overfitting by smoothing the label distribution, acting as label regularization that reduces the effective learning rate on high-confidence examples",
        "Temperature scaling makes the loss function convex, guaranteeing convergence to the global optimum during student training by eliminating all non-convex saddle points and local minima"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: ["Using a smaller teacher model that closely matches the student's capacity, reducing the capacity gap to provide a more achievable learning target and improving overall knowledge transfer", "Having the **student generate its own outputs**, then using the teacher to score/correct them — avoiding the distribution mismatch where the student trains on teacher text but self-generates at inference", "Training without any teacher signal during generation, relying solely on the student's self-supervised next-token prediction objective to learn useful representations from its own generated output sequences", "Using only hard labels from the teacher instead of soft probability distributions, removing the temperature hyperparameter entirely and reducing the distillation loss to a standard cross-entropy formulation"],
      correct: 1,
    ...
    },
    {
      ...
      options: ["The same as the teacher's cost because the student must internally simulate the teacher's full computation graph to reproduce the distilled knowledge accurately at inference time", "95% of the teacher's cost, since the distillation process transfers most of the teacher's computational requirements into the student's learned weight representations and activations", "50% of the teacher's cost due to shared architecture design, where the student reuses half of the teacher's transformer layers directly during the forward pass at inference", "~10% of the teacher's cost — inference cost scales roughly linearly with parameter count (dominated by memory bandwidth for decode), so a 10x smaller model is approximately 10x cheaper"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["The **training cost of each candidate architecture** is enormous (millions of dollars for a full pre-training run), making exhaustive search infeasible — LLM NAS relies on proxy metrics or constrained subspaces", "LLM architectures are already optimal due to extensive manual tuning over the past decade, leaving no room for automated improvement through search-based methods exploring the full design space", "LLMs don't have meaningful hyperparameters to search over since the Transformer architecture has converged to a standard fixed design with predetermined depth, width, head count, and FFN ratios", "NAS requires supervised labels to evaluate each candidate architecture during the search process, which LLMs cannot provide since they rely exclusively on self-supervised next-token prediction"],
      correct: 0,
