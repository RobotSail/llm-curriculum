  ...
  steps: [
    {
      ...
      options: ["The loss function of LLMs is not twice-differentiable due to the discrete argmax in token selection, making the Hessian undefined and preventing computation of the required curvature matrix", "The gradient $\\nabla_\\theta \\ell(z)$ vanishes at the optimum for overparameterized models, so the influence function evaluates to zero for any training example regardless of its importance", "Influence functions require the model to be trained to full convergence on the training set, which is deliberately avoided in LLM pretraining where models see each example only once", "Computing or approximating the inverse Hessian $H_\\theta^{-1}$ is intractable for billions of parameters, and the quadratic approximation breaks down in the non-convex, overparameterized regime"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Using only the first-order gradient without any Hessian information, computing a simple dot product between training and test gradients as an efficient but approximate influence proxy score", "Training a separate neural network to predict influence scores directly from input features, bypassing the need to differentiate through the original model or compute any gradient information", "Projecting per-example gradients into a low-dimensional random subspace, then computing attribution scores via a linear model in that projected space using tractable random projections", "Computing influence only for the last transformer layer of the model, where gradients are largest and most informative about the mapping from internal representations to output predictions"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Resetting the learning rate to its initial maximum and repeating the full warmup and decay schedule from scratch, treating the continuation as an entirely new pretraining run", "Using a constant learning rate throughout continual pretraining, maintaining a steady adaptation rate so the model adjusts uniformly across the entire new data distribution", "Briefly increasing the learning rate to a moderate value before decaying again, helping the model escape its current loss basin and adapt to the new distribution", "Reducing the learning rate to near zero throughout adaptation to prevent catastrophic forgetting, ensuring the model's weights change minimally while still processing new data"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["It cannot process documents longer than the reference model's context window, causing systematic exclusion of long-form content such as technical papers, legal documents, and books", "It is too computationally expensive to apply at web scale because computing perplexity requires a full forward pass of the reference model over every single candidate document", "It removes all non-English text regardless of its actual quality, since the reference language model assigns high perplexity to any text written in an unfamiliar language", "It systematically biases the corpus toward the style and domain of the reference model's training data — a Wikipedia-trained filter favors encyclopedic prose and discards informal but informative content"],
      correct: 3,
    ...
    },
    {
      ...
      options: [
        "Use only domain-specific data to maximize specialization, since the general capabilities from pretraining are robust enough to persist without any reinforcement or mixing",
        "Use only general data but increase the learning rate substantially to capture domain knowledge from the few relevant examples that naturally appear in the general corpus",
        "Mix domain-specific data with general-purpose data, tuning the ratio empirically — too much domain data causes forgetting, too little yields insufficient specialization",
        "Alternate between pure domain and pure general data in separate sequential training phases, letting the model fully adapt to each distribution before switching to the other"
      ],
      correct: 2,
