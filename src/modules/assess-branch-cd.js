  ...
  steps: [
    {
      ...
      options: ["CoT activates special reasoning circuits in the transformer that are dormant during standard next-token prediction and require explicit prompting to engage", "CoT prompts contain the answer implicitly within their structure, and the model merely extracts it through pattern matching on the exemplar format", "The intermediate tokens serve as **external working memory** — they allow the model to decompose multi-step problems into sequential single-step computations, each conditioned on the previous result, effectively extending the model's bounded computational depth per token", "Writing more tokens gives the model more time to think in wall-clock time, and the additional GPU processing cycles accumulate into deeper reasoning"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["PRMs provide **dense, step-level credit assignment** — they can identify exactly where a reasoning chain went wrong, enabling more efficient search over reasoning paths and better training signal for the policy model, rather than assigning a single reward to an entire multi-step derivation", "PRMs are cheaper to train because they use shorter input sequences, with each step evaluated independently rather than requiring full-chain processing", "PRMs always produce more accurate final answers because step-level verification guarantees that every intermediate result is correct before proceeding", "PRMs don't require human annotations since correctness of individual reasoning steps can be verified automatically through symbolic execution"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["Whether LLMs use symbolic or subsymbolic representations internally, since only symbolic representations can support true logical inference and deduction", "Whether LLMs were trained on sufficient reasoning data to acquire genuine reasoning capabilities, or whether more training data would bridge the gap", "Whether LLMs use more parameters than the human brain has synapses, since computational capacity determines the boundary between pattern matching and reasoning", "Whether LLMs exhibit **systematic generalization** — solving novel problem compositions they haven't seen in training — or merely interpolate between training examples. Key evidence includes failure on problems with superficially similar structure but different solutions, and sensitivity to irrelevant surface features"],
      correct: 3,
    ...
    },
    {
      ...
      options: [
        "It has a built-in calculator module that activates when arithmetic expressions are detected in the token stream during generation",
        ...
        "Transformers can natively perform multiplication through the interaction of attention patterns and feed-forward layers at sufficient depth",
        "The chain-of-thought prompt forced the correct algorithm by constraining the generation to follow the specific decomposition pattern shown in the examples"
      ],
      correct: 1,
  ...
  steps: [
    {
      ...
      options: [
        "Running the model on multiple GPUs simultaneously to perform parallel beam search over the top-$k$ most likely token sequences",
        ...
        "Fine-tuning the model on each new question using a few gradient steps of in-context adaptation before generating the response",
        "Searching over a database of pre-computed answers indexed by question embeddings and returning the closest semantic match"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: ["Can select based on **response quality rather than just answer frequency** — it can prefer a well-reasoned minority answer over a frequently-repeated shallow answer, but its effectiveness is bounded by the verifier's accuracy and the coverage of $N$ samples", "Always produces better results because the reward model provides a strictly more informative selection signal than answer frequency counting", "Uses less compute since it only generates one response and applies the reward model as a cheap post-hoc filter on that single output", "Is identical to self-consistency in both mechanism and outcome, since the reward model's ranking always agrees with the majority vote result"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["LLMs can't play games because they lack the spatial reasoning capabilities that MCTS requires for effective position evaluation", "MCTS requires a board representation with discrete legal moves, and natural language reasoning cannot be decomposed into a finite action space", "The **branching factor is enormous** — at each token (or reasoning step), there are thousands of possible continuations, making exhaustive tree search intractable; games like Go have ~250 legal moves per position, while an LLM's vocabulary is 32K-128K tokens, and even at the reasoning-step level, the space of possible next thoughts is vast", "LLMs are too slow for real-time search because each node evaluation requires a full forward pass through billions of parameters"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Only the model size — larger models should always think longer because they have more representational capacity to utilize during extended reasoning", "The time of day and current server load, since compute allocation should be dynamically adjusted based on available infrastructure capacity", "The user's subscription tier, since higher-paying users should receive more inference compute regardless of the difficulty of their query", "The **difficulty of the specific input** — easy questions should use minimal test-time compute (direct answering), while hard questions benefit from extended reasoning, search, or multiple samples; spending equal compute on all inputs is wasteful"],
      correct: 3,
  ...
  steps: [
    {
      ...
      options: ["Hard-coding specific API calls into the model's architecture through specialized attention heads that route queries to external function endpoints", "Giving the model direct access to execute code during training so it learns the input-output behavior of each function through trial and error", "Fine-tuning on datasets of (user query, function call, function result, final response) sequences — the model learns to emit **structured function call tokens** in a special format, which are intercepted by the serving system, executed, and the results injected back into the context", "Training a separate classifier that maps user queries to the appropriate function based on intent detection, then invoking the function independently"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Larger chunks are always better because they provide more context for the LLM to synthesize its answer from each retrieved passage", "All documents should be stored as single chunks to preserve the full semantic coherence of each source and avoid fragmenting arguments", "Chunking doesn't matter if the embedding model is good enough, since a powerful embedder can represent any chunk size with equal fidelity", "Chunk size involves a **precision-recall trade-off**: smaller chunks (128-256 tokens) yield precise retrieval but may miss surrounding context, while larger chunks (512-1024 tokens) capture more context but may dilute relevance with irrelevant text; overlapping chunks and hierarchical strategies can mitigate this"],
      correct: 3,
    ...
    },
    {
      ...
      options: [
        "It uses a larger embedding dimension that captures more nuanced semantic distinctions than the compact vectors used by the initial bi-encoder retriever",
        ...
        "It has access to more training data because reranker models are trained on the union of all retrieval datasets rather than a single domain-specific corpus",
        "It runs faster than the initial retriever because it only processes the top-100 candidates rather than searching over the entire document collection"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: ["The answer requires **synthesizing information from multiple documents** or requires a chain of lookups — e.g., \"What is the GDP of the country where the inventor of the transformer architecture was born?\" requires first retrieving who invented transformers, then their birthplace, then that country's GDP", "The document collection is very large, exceeding the capacity of the embedding index to return relevant results in a single retrieval step", "The embedding model has limited context length and cannot encode queries that are longer than a few hundred tokens into a single vector representation", "The user asks multiple questions in one turn, requiring the system to retrieve separate sets of documents for each independent sub-question"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["Never — more context is always better because the model can simply ignore irrelevant passages and focus on the useful information in the retrieved set", "Only when the context window is physically exceeded, since the model benefits from all available information up to the maximum sequence length", "After exactly 3 passages in all cases, since empirical studies have converged on this as the universal optimal number for retrieval-augmented generation", "When additional passages have **low relevance scores** — they introduce noise and irrelevant information that can distract the LLM, cause hallucinated synthesis of contradictory sources, or push relevant information into the less-attended middle of the context window. Optimal passage count depends on query type and model capability"],
      correct: 3,
  ...
  steps: [
    {
      ...
      options: [
        "Training and inference steps, where the agent alternates between updating its weights on new observations and generating predictions based on the updated model",
        ...
        "Encoding and decoding steps, where the encoder processes the environment state and the decoder generates the appropriate action sequence in response",
        "Forward and backward passes through the model, where the forward pass generates an action and the backward pass updates the policy based on the observed reward"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: ["GPU memory vs CPU memory, with high-priority recent observations stored on the faster GPU and historical data offloaded to the CPU for retrieval when needed", "Attention keys vs attention values within the transformer, with keys serving as indexable short-term memory and values storing the long-term content retrieved through attention", "Training data vs inference data, where the model's parametric memory from training serves as long-term storage and the inference-time context provides short-term task information", "The **context window** (short-term: recent observations, current plan, working state) and **external storage** (long-term: vector databases of past experiences, summarized episode histories, learned procedures) — the context window is limited and ephemeral, while external storage persists across sessions"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Complete the booking and hope the date is correct, since backtracking would waste the compute already invested in the preceding steps", "Start over from step 1 after every step to ensure a clean state, accepting the computational overhead as the cost of reliable execution", "Include a **verification step before irreversible actions** — after filling in details (step 3-4), verify the entered information against the original request before proceeding to payment; irreversible actions like payment should always be preceded by explicit confirmation steps that check for accumulated errors", "Use a separate specialized model for each step, since task-specific models make fewer errors than a single general-purpose agent"],
      correct: 2,
    ...
    },
    {
      ...
      options: [
        "The agent might generate offensive text in its reasoning traces, which could be exposed to users through logging or debugging interfaces",
        ...
        "The agent will use too much compute by entering infinite reasoning loops, consuming expensive GPU resources without producing useful output",
        "The agent's actions are too slow for real-time applications, since each tool call adds latency that compounds across the multi-step execution"
      ],
      correct: 1,
