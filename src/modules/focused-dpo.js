...
export const dpoLearning = {
  ...
  steps: [
    ...
    // Step 12: MC — DPO vs PPO tradeoffs
    {
      ...
      options: [
        "The preference pairs are too easy — both $y_w$ and $y_l$ are high-quality responses with only subtle differences, so learning to distinguish between them doesn't push the model toward meaningfully better generation behavior",
        ...
        "The $\\beta$ parameter is too large, causing the KL penalty to dominate the objective and prevent any meaningful deviation from the reference policy's generation behavior, effectively nullifying alignment",
        "DPO fundamentally cannot improve generation quality — it only reshapes the model's implicit reward landscape without altering the autoregressive generation distribution that determines actual output quality"
      ],
      correct: 1,
    ...
    // Step 14: MC — integration
    {
      ...
      options: [
        "The 50K preference pairs are insufficient for a 70B model — DPO requires at least 500K pairs for models above 10B parameters to achieve statistical convergence across the full output distribution, so this dataset is an order of magnitude too small",
        "DPO's gradient computation is $O(n^3)$ in sequence length due to the log-probability ratio calculation requiring a full Jacobian over the vocabulary, making it computationally prohibitive for the long responses typical of 70B-scale generation",
        "The 70B model's vocabulary is too large for the softmax computation in the implicit reward — computing log-probabilities over 128K+ tokens at every position requires specialized sparse approximations that degrade alignment quality significantly",
