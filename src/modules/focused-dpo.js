...
export const dpoLearning = {
  ...
  steps: [
    ...
    // Step 2: MC — RLHF objective understanding
    {
      ...
      options: [
        "The policy converges to the reference policy $\\pi_{\\text{ref}}$, since the reward signal becomes negligible compared to the KL anchor",
        "The policy maximizes the reward model without constraint, likely exploiting artifacts in the reward model to generate degenerate high-reward text — a phenomenon called reward hacking",
        "The objective becomes equivalent to supervised fine-tuning on the highest-reward responses in the training set",
        "Training becomes numerically unstable because the KL divergence denominator approaches zero, producing infinite gradients"
      ],
      correct: 1,
    ...
    // Step 4: MC — optimal policy
    {
      ...
      options: [
        "Only responses with reward above the mean reward $\\bar{r}(x) = \\mathbb{E}_{y \\sim \\pi_{\\text{ref}}}[r(x,y)]$ — all below-mean responses are assigned exactly zero probability",
        "All responses are amplified equally — the exponential factor is absorbed by the normalizing constant $Z(x)$, leaving the distribution unchanged",
        "Responses with higher reward get exponentially more probability mass relative to the reference, but even low-reward responses retain some probability since $\\exp(r/\\beta) > 0$ for any finite reward",
        "Only the single highest-reward response gets any probability mass, since the exponential amplification creates a winner-take-all dynamic regardless of $\\beta$"
      ],
      correct: 2,
    ...
    // Step 6: MC — the derivation
    {
      ...
      options: [
        "The Bradley-Terry model normalizes rewards internally, making any additive constant irrelevant — since $Z(x)$ contributes the same additive constant $\\beta \\log Z(x)$ to both $r_w$ and $r_l$, it drops out of the difference",
        "The partition function $Z(x)$ is always exactly 1 when the reference policy is properly normalized, so $\\log Z(x) = 0$ and there is nothing to cancel",
        "The sigmoid function $\\sigma$ has the property that $\\sigma(a + c) - \\sigma(b + c) = \\sigma(a) - \\sigma(b)$ for any constant $c$, which absorbs the partition function",
        "DPO approximates $Z(x) \\approx 1$ using a Taylor expansion, and the cancellation is approximate rather than exact — this introduces a small bias that is negligible in practice"
      ],
      correct: 0,
    ...
    // Step 12: MC — DPO vs PPO tradeoffs
    {
      ...
      options: [
        "The preference pairs are too easy — both $y_w$ and $y_l$ are high-quality, so learning to distinguish them doesn't improve generation",
        "The model has overfit the implicit reward to the specific responses in the training set, but due to distribution shift, the learned preferences don't transfer to the responses the model actually generates during inference",
        "The $\\beta$ parameter is too large, causing the KL penalty to prevent any meaningful deviation from the reference policy's generation behavior",
        "DPO fundamentally cannot improve generation quality — it only changes the model's reward function, not its generation distribution"
      ],
      correct: 1,
