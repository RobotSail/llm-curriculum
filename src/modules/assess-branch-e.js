...
export const vlmAssessment = {
  ...
  steps: [
    ...
    },
    {
      ...
      options: ["It trains the visual encoder from scratch to recognize objects, since the frozen CLIP encoder lacks the fine-grained spatial understanding needed for conversational visual tasks", "It is only necessary for projection-based models that lack cross-attention, since cross-attention models already learn visual grounding implicitly through their architectural inductive bias", "It replaces the contrastive pretraining stage of CLIP entirely, providing a more task-relevant visual representation learned end-to-end from conversational image-text pairs", "Without it, the model can embed visual tokens but has not learned to reason about, describe, or follow instructions involving visual content -- the projection layer alone provides spatial alignment but not task competence"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Attention cost increases by $16\\times$ -- self-attention has $O(n^2)$ complexity in sequence length, and the visual token count itself scales quadratically with resolution, so doubling resolution yields a $4\\times$ token increase and $16\\times$ attention cost relative to the original", "Attention cost increases by $2\\times$ -- self-attention scales linearly with token count since each token attends to a fixed-size local window determined by the patch size, not the full sequence", "Attention cost remains unchanged because the patch size is the same and each patch undergoes identical computation regardless of how many other patches exist in the sequence", "Attention cost increases by $8\\times$ -- the token count quadruples but the attention mechanism only computes pairwise interactions within each row of the spatial grid, not across the full sequence"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["Each crop loses all global context since crops are processed independently, making holistic scene understanding impossible even with the global view providing only a low-resolution summary", "Tiling requires retraining the visual encoder from scratch on cropped image distributions, since a ViT pretrained on full images produces degraded features when applied to partial crops", "The total visual token count becomes very large (e.g., $6 \\times 1024 + 256 \\approx 6400$ tokens), consuming a significant fraction of the LLM's context window and proportionally increasing inference cost", "The crops overlap at their boundaries by design, creating redundant tokens at shared edges that confuse the model's spatial reasoning and introduce inconsistent duplicate representations"],
      correct: 2,
  ...
  steps: [
    {
      ...
      options: ["The composition of Gaussian transitions is itself Gaussian: $q(x_t | x_0) = \\mathcal{N}(x_t; \\sqrt{\\bar{\\alpha}_t}\\, x_0,\\, (1-\\bar{\\alpha}_t)I)$ where $\\bar{\\alpha}_t = \\prod_{s=1}^{t}(1-\\beta_s)$, so any timestep can be sampled in closed form", "The noise added at each step is correlated with previous steps via a shared random seed, enabling recursive cancellation of intermediate noise terms through the Markov property", "The forward process follows a deterministic ODE trajectory rather than a stochastic SDE, so the path from $x_0$ to $x_t$ is uniquely determined without requiring intermediate samples", "Each step adds exactly the same amount of noise $\\beta$ regardless of the timestep, making the total noise after $t$ steps simply $t \\cdot \\beta$ and the closed-form expression trivial"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["Higher $w$ accelerates convergence during the reverse diffusion process, reducing the wall-clock time per sample at the cost of decreased numerical stability in the noise prediction estimates", "Higher $w$ increases the effective output resolution of the generated image by sharpening high-frequency spatial details, while simultaneously reducing the dynamic range of color values in low-frequency regions", "Higher $w$ pushes samples toward modes of the conditional distribution, increasing text-image alignment and perceived quality at the cost of reduced diversity -- extreme $w$ produces saturated, over-sharpened images", "Higher $w$ reduces the number of denoising steps required for convergence by amplifying the signal-to-noise ratio at each step, enabling faster sampling with fewer function evaluations of the noise prediction network"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Training a lightweight distilled network to approximate multiple U-Net denoising steps in a single forward pass, amortizing the cost of iterative refinement into a faster student model", "Progressively reducing the spatial resolution of the image at intermediate denoising timesteps and upsampling only at the final step, so the U-Net operates on smaller tensors for most of the reverse process", "Incorporating a GAN discriminator that evaluates intermediate denoised images and skips remaining denoising steps once the sample is judged to be of sufficient perceptual quality by the discriminator", "Redefining the reverse process as a non-Markovian deterministic mapping that shares the same marginals $q(x_t|x_0)$ as DDPM but allows skipping timesteps, producing a deterministic ODE trajectory from noise to image"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Straight paths minimize transport cost and enable high-quality generation in very few steps (sometimes 1--4), because the learned ODE trajectories have low curvature and the Euler discretization error is small", "Straight-path transport eliminates the need for a neural network entirely, since the linear interpolation between noise and data can be computed analytically without any learned parameters", "The straight-path formulation is only applicable to images smaller than $256 \\times 256$, because the linear transport assumption breaks down at higher resolutions where the data manifold has more complex curvature", "Straight-path transport replaces the U-Net with a single linear transformation from noise to data space, since the velocity field along straight trajectories is constant and requires no nonlinear function approximation"],
      correct: 0,
  ...
  steps: [
    {
      ...
      options: ["The highest-frequency components of the audio waveform, with subsequent codebooks capturing progressively lower frequency bands in a spectral decomposition hierarchy", "Only the silence and non-silence boundary transitions in the audio, with subsequent codebooks filling in the spectral content within each detected speech segment", "The coarsest approximation of the signal, with each subsequent codebook encoding the residual error from previous quantization stages, progressively refining the reconstruction", "Randomly selected non-overlapping frequency bands from the mel spectrogram, with each codebook independently responsible for a disjoint portion of the frequency spectrum"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["Prosody is a phenomenon exclusive to tonal languages like Mandarin and Cantonese, so tokenizers trained primarily on English speech discard pitch variation as noise rather than encoding it as a meaningful feature", "Prosody is fully determined by the text content itself and does not need to be separately encoded in the audio tokens, since a competent language model can reconstruct the correct intonation pattern from the words alone", "Modern discrete speech tokenizers with sufficiently large codebooks perfectly capture all prosodic features including pitch, stress, and rhythm, since the codebook entries span the full space of acoustic variation", "Discrete codebooks with limited size force prosodic variation into a finite set of categories, quantizing continuous $F_0$ (fundamental frequency) contours and energy envelopes into coarse bins -- subtle distinctions like sarcasm, hesitation, or emphasis that depend on fine-grained pitch and timing modulation may fall within the same quantization bucket"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["Perfectly preserve all emotional cues in the learned representations, since recognizing the speaker's emotional state is essential for correctly transcribing words that are pronounced differently under different affects", "Capture emotional information exclusively in the first RVQ level alongside the semantic content, since emotion and phonemic identity are inherently entangled in the acoustic signal and cannot be separated by any training objective", "Discard emotional information because ASR objectives reward tokens that are invariant to speaker affect -- the same phoneme should map to the same token whether spoken angrily or sadly, so the encoder actively learns to suppress paralinguistic variation that does not aid transcription", "Produce emotional representations that are more perceptually accurate than human listeners, since neural speech encoders trained at scale learn to detect subtle affective cues in pitch and timing that are below the threshold of conscious human perception"],
      correct: 2,
  ...
  steps: [
    {
      ...
      options: ["Generating each frame independently at very high resolution so that the increased per-frame detail provides enough visual consistency cues for human perception to interpolate smooth motion", "Limiting generation to very short clips under 0.5 seconds, since temporal coherence degrades proportionally with duration and short clips stay within the model's consistency radius", "Applying a separate post-processing neural network trained on optical flow estimation to smooth out temporal inconsistencies by warping each frame to align with its neighbors after generation", "Extending the 2D spatial attention of image diffusion models with temporal attention layers that attend across frames at corresponding spatial positions, allowing the model to enforce coherence of objects, lighting, and motion across the time dimension"],
      correct: 3,
    ...
    },
    {
      ...
      options: ["~101,250 tokens (450 temporal slots $\\times$ 225 spatial tokens per frame pair), which exceeds the 32K--128K context windows of most LLMs, requiring either aggressive compression, sparse attention, or hierarchical processing", "~2,000 tokens (aggressive spatial and temporal pooling reduces the representation to roughly 4--5 tokens per frame), fitting comfortably within any standard context window without compression", "~1,000,000 tokens (each pixel contributes roughly one token even after patching), requiring a completely new architectural paradigm beyond what current transformers can handle", "~10,000 tokens (moderate spatial compression yields roughly 22 tokens per temporal slot), which is manageable with standard full self-attention and does not require specialized efficiency techniques"],
      correct: 0,
