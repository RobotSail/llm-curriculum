...
export const ppoMechanicsLearning = {
  ...
  steps: [
    ...
    },
    {
      ...
      options: [
        "The reward model's score was likely miscalibrated for such a rare response, producing an inflated advantage signal",
        "The advantage estimate used a value function trained on the old policy, so the baseline is stale and inapplicable",
        "Assigning 0.8 probability to one response necessarily collapses the entropy budget across all other outputs",
        "The gradient was estimated at probability 0.1 — it is only valid for small local changes, not a jump to 0.8"
      ],
      correct: 3,
    ...
    {
      type: "mc",
      question: "In the TRPO objective, the importance weight $r_t = \\pi_\\theta(y) / \\pi_{\\text{old}}(y)$ reweights advantages from the old policy. For a response with positive advantage, the ratio grows to $r_t = 3.0$ after several gradient steps. What does this signal about the optimization?",
      options: [
        "The advantage estimate $\\hat{A}$ was underestimated by a factor of 3, and the ratio compensates by amplifying the gradient proportionally",
        "The new policy assigns 3x the probability to this response compared to the old policy — the update has moved far outside the region where the advantage estimate is reliable",
        "The value function baseline has shifted by a factor of 3, so the effective advantage is now $\\hat{A}/3$ after normalization",
        "The response was generated by a policy 3 iterations old rather than the current $\\pi_{\\text{old}}$, so the ratio measures staleness across multiple rollout cycles"
      ],
      correct: 1,
      explanation: "The ratio $\\pi_\\theta(y) / \\pi_{\\text{old}}(y) = 3.0$ means the new policy assigns 3x the probability to response $y$ compared to the policy that generated it. This large ratio means the policy has changed substantially for this response — which is exactly what trust region methods try to limit, since our advantage estimate $A(y)$ was computed under $\\pi_{\\text{old}}$ and is only a local approximation. PPO's clipping mechanism would cap this at $1 + \\epsilon$ (e.g., 1.2) to prevent such large deviations."
    },
    {
      type: "info",
    ...
    },
    {
      ...
      options: [
        "The gradient is proportional to $\\hat{A} = 1.5$ because the unclipped surrogate still dominates the $\\min$ operator",
        "The gradient is zero because $r_t = 1.4 > 1 + \\epsilon = 1.2$, so the clipped term is active and flat",
        "The gradient is proportional to $1.2 \\times 1.5 = 1.8$ since the clipped term provides a scaled-down signal",
        "The gradient is negative because exceeding the trust region reverses the optimization direction entirely"
      ],
      correct: 1,
    ...
    },
    {
      ...
      options: [
        "The clipped responses contribute zero gradient, so later epochs effectively only update on responses still within range",
        "The clipped responses are removed from the batch entirely, reducing the effective batch size by a growing fraction",
        "The clipping threshold is relaxed in later epochs by a factor of $K$ to allow the policy to keep improving",
        "All responses contribute equally regardless of clipping since the gradients are averaged across all epochs"
      ],
      correct: 0,
    ...
    },
    {
      ...
      options: [
        "The policy immediately snaps back to $\\pi_{\\text{ref}}$ because the 5x penalty increase dominates all reward signal",
        "The PPO clipping becomes ineffective because the KL gradient overwhelms the clipped surrogate objective",
        "Nothing changes because the KL penalty is evaluated against $\\pi_{\\text{old}}$, not the original $\\pi_{\\text{ref}}$",
        "The policy gradually drifts back toward $\\pi_{\\text{ref}}$ as the stronger penalty outweighs marginal reward gains"
      ],
      correct: 3,
    ...
    },
    {
      ...
      options: [
        "Reducing all learning rates uniformly because the average gradient magnitude decreases across the full batch",
        "Ignoring the clipped responses entirely, which is the intended behavior and introduces no optimizer artifacts",
        "Increasing effective learning rates for unclipped parameters as their second-moment estimates shrink from sparse gradients",
        "Accumulating a large momentum in the KL penalty direction since the KL term is never subject to clipping"
      ],
      correct: 2,
